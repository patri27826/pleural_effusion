...
class UNet:
    def __init__(self, image_size, learning_rate):
        self.input_size = (image_size[0], image_size[1], 1)
    ...
    def build_model(self, filters):
        ...
        model.compile(optimizer=tensorflow.keras.optimizers.Adam(learning_rate=self.learning_rate), loss=self.dice_coef_loss, metrics=[self.dice_coef])
        return model
